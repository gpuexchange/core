describe('TransactionsCache', () => {
    it('correctly finds transactions', (done) => {
        (async () => {
            ...
            // Duplicate
            expect(testBlockchain.transactionsCache.missingBlocks).toBe(Policy.TRANSACTION_VALIDITY_WINDOW - 15);
            const cache = testBlockchain.transactionsCache.duplicate();
            expect(cache.missingBlocks).toBe(Policy.TRANSACTION_VALIDITY_WINDOW - 15);
